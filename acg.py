...
def downloadAlbum(albumName, albumIndex, downloadUrl, downloadFile):
    reqs = []
    try:
        res = urllib.request.urlopen(urllib.request.Request(downloadUrl + "/index?index=" + albumIndex + "&mode=7"))
...
def downloadIndex(downloadUrl, downloadFile):
    ...
    try:
        print(downloadUrl + "/index?index=mainindex&mode=7&isv=1")
    ...
    for Album in resAlbum['indexes']:
        ...
        # 下载专辑
        downloadAlbum(AlbumName, Album['index'], downloadUrl, downloadFile)
...
if __name__ == "__main__":
    # acgstay 1 .. 5
    # 下载地址
    downloadUrl = "http://acgstay2.mavericks.lol:8888"
    # 下载文件夹
    downloadFile = "ACG"
    # 线程任务
